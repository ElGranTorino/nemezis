...
const DiscountCode: React.FC<DiscountCodeProps> = ({ cart }) => {
  ...
  return (
    <div className="w-full bg-white flex flex-col">
      <div className="mb-4">
        <h3 className="text-base-semi">Промокод на знижку</h3>
      </div>
        ...
        {appliedDiscount ? (
          <div className="flex items-center justify-between">
            ...
            <div>
              ...
              >
                <Trash size={16} />
                <span className="sr-only">Видалити купон на знижку</span>
              </button>
            </div>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-[1fr_80px] gap-x-2">
              <Input
                ...
                {...register("discount_code", {
                  required: "Необхідно вказати код",
                })}
                errors={errors}
              />
              <div>
                <Button
                  className="!min-h-[0] h-[46px]"
                  style={{padding: 'auto 1rem auto'}}
                  disabled={isLoading}
                  isLoading={isLoading}
                >
                  Ок
